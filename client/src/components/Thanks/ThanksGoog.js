...
const ThanksGoog = () => {
  ...
  let adData = JSON.parse(localStorage.getItem('axrateQsearch'))
  // console.log(adData);
  ...
  if (!listings) {
    return (
      <div className='search-no-ads'>
        <h1 className='search-ad-title'>Sorry! There are currently no offers for your area at this time, try a different zipcode or try again later.</h1>
        <button onClick={() => window.location.href = '/get-quote/nat'} className='search-back-btn'>Back</button>
    ...
    return (
      <div>
        <div className='search-ad-container'>
          <h1 className='search-ad-title'>Compare Insurance Quotes in Your Area. Click on 2 or more Insurance Partners below to compare quotes and maximize your savings.</h1>
          <div className='search-ads'>
            {listings.map(ads => (
              <div className='search-ad' key={ads.buyerID}>
                <img src={ads.logo} alt={ads.displayname} />
                <div>
                  <p className='search-listing-title'>{ads.listingTitle}</p>
                  {/* <div>{ads.listingDescription.replace(/(<([^>]+)>)/gi, "").replace(/([A-Z])/g, ' $1').trim()}</div> */}
                </div>
                <button onClick={() => window.location.href = ads.url} className='search-get-quote-btn'>GET QUOTE</button>
                <iframe title='pixel' src="https://www.a1jkghtrk.com/?nid=1715&oid=1325" scrolling="no" frameborder="0" width="1" height="1"></iframe>
              </div>
            ))}
          </div>
